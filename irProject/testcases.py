...
class TestIndexer(unittest.TestCase):
    ...
    def test_cosine_similarity(self):
        ...
        _, tfidf_matrix, _ = load_index()
        similarities = search('courage', _, tfidf_matrix)
...
class TestQueryProcessor(unittest.TestCase):
    def test_query_response(self):
        ...
        response = requests.post('http://localhost:5000', data={'query': 'courage'})
        self.assertEqual(response.status_code, 200)
        self.assertIn('beauty-restored-riley', response.text)  # Assuming 'results' is part of the response HTML when results exist
        print("testcase3 end............")

    def test_invalid_query_handling(self):
        print("testcase4 start.............")
        """Test error handling for invalid queries."""
        response = requests.post('http://localhost:5000', data={'query': ''})
